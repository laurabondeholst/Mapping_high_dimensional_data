...
for perc_i, percentile in enumerate(percentiles):
    ...
    for rep in range(repetitions[perc_i]):
        ...
        kmeans.run_kmeans(Y, labels_split, percentile, perc_error, perc)
    #   
    curr_error = np.mean(perc_error)
    error.append(curr_error) 
    print(f"Mean error for percentile {percentile} is: {curr_error}")
    # save error for percentile


fig = go.Figure()               
fig.add_trace(go.Scatter(x=percentiles, y=error,
                    mode='lines'
                    ))
